...
class EmailActionsController < BaseController
  ...
  post '/email/reset' do
    ...
    action = EmailAction.create(user['id'], 'reset_password')
  ...
  get '/email/confirm/:code' do
    ...
    halt 404, { error: 'Wrong action type' }.to_json unless action['action_type'] == 'confirm_account'

    User.confirm!(action['user_id'])
  ...
  post '/email/reset-password/:code' do
    ...
    action = EmailAction.find_by_code(code)
    halt 404, { error: 'Invalid or expired code' }.to_json unless action
    halt 404, { error: 'Wrong action type' }.to_json unless action['action_type'] == 'reset_password'
